...
구조: 예시 글처럼 자연스러운 흐름. 아래 요소 포함:
1. 서론 (방문 계기 + 해시태그 3개, 첫 3줄에 핵심 키워드)
2. [운영정보] 블록
3. 내부 분위기 (2~3줄)
4. 주문한 메뉴별 리뷰 (솔직 감상)
5. 아쉬운 점 1개 (D.I.A가 신뢰도 높게 평가)
6. 총평 + 재방문 의사
7. 해시태그 7개

체류시간 극대화 구조 (D.I.A+ 핵심 지표):
- 첫 3줄에 공감 문장 or 질문 ("~고민이시죠?", "~찾고 계신다면!")
- 3문장 이상 이어지는 문단 금지 (모바일 가독성)
- 300~500자마다 소제목 삽입
- 중간에 독자 질문 1~2개 ("여러분은 ~파?")
- 끝에 다음 글 예고 ("다음엔 ~도 가볼 예정이에요!")

금지:
- **, ##, --- 등 마크다운 문법
- "다양한", "풍부한", "맛의 하모니", "미각의 향연"
- ~합니다, ~됩니다 (격식체)
- 같은 키워드 4회 이상 반복 (스팸 판정 위험)
- ✏️ 표시나 수정 안내

필수:
- [사진: 말투로 설명] 형식 5~10곳 (과하지 않게, 다양한 앵글)
- [움짤: 설명] 1~2곳
- 1~2문장마다 줄바꿈
- 전체 1500~2000자 (D.I.A 최적 범위, 과하면 역효과)
- 소제목은 이모지+텍스트 (📍, 🍽, 🍺, 🔚 등)
...
공식: [핵심키워드] + [가게명] + [보조키워드or감성]
예: "모란맛집 진솔정 곱창전골 솔직후기"

### 본문
(예시 글과 같은 말투의 완성된 블로그 본문)

### 해시태그
(7개, 공백 구분, 한 줄. 핵심2~3 + 세부3~4 + 트렌드1)
"""
    return prompt
