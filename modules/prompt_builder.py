...
def _build_place_info(place_detail: dict = None) -> str:
    ...
    lines = ["[운영정보 - 아래 내용을 [운영정보] 블록에 그대로 복사할 것]"]

    addr = place_detail.get("road_address") or place_detail.get("address", "")
    lines.append(f"위치 : {addr or '확인 필요'}")
    lines.append(f"운영시간 : {place_detail.get('business_hours', '확인 필요')}")

    if place_detail.get("last_order"):
        lines.append(f"라스트오더 : {place_detail['last_order']}")

    lines.append(f"전화번호 : {place_detail.get('telephone', '확인 필요')}")

    if place_detail.get("reservation"):
        lines.append(f"예약 : {place_detail['reservation']}")

    if place_detail.get("takeout") or place_detail.get("delivery"):
        takeout = place_detail.get("takeout", "")
        delivery = place_detail.get("delivery", "")
        lines.append(f"포장, 배달 : {', '.join(filter(None, [takeout, delivery])) or '확인 필요'}")

    if place_detail.get("parking"):
        lines.append(f"주차 : {place_detail['parking']}")

    restroom = place_detail.get("restroom_info", [])
    if restroom:
        lines.append(f"화장실 : {' / '.join(restroom)}")
...
키워드별 동의어/변형 (자동 순환 삽입할 것):
- "{kw1}" 변형: "{kw1}", "{kw1}추천", "{kw1}후기", "{kw1}리뷰"
- "{kw2}" 변형: "{kw2}", "{kw2} 추천", "{kw2} 후기"
- "{kw3}" 변형: "{kw3}", "{kw3} 후기"
- "맛집" 변형: "맛집", "맛집추천", "가볼만한곳", "맛집후기", "맛집투어"

키워드 삽입 규칙 (D.I.A+ 최적화):
- 전체 키워드 밀도 1.5~2% (과하면 스팸 판정)
- 1위 키워드 + 변형: 서론 첫 문장 + 소제목 1개 + 마무리 = 최소 3회
- 2~3위 키워드 + 변형: 각 2회
- 동일한 형태를 연속 사용하지 말고, 변형을 돌려가며 쓸 것
- 동일 키워드 연속 2문장에 절대 넣지 말 것

[작성 규칙 - 네이버 에디터 형식 + 2026 SEO]

아래 템플릿 구조를 정확히 따를 것. 각 섹션의 순서, 사진 자리, 줄바꿈을 동일하게 복제.

=== 필수 템플릿 구조 ===

[1. 서론 - 인사 + 키워드 질문 + 방문 소개]
- 인사 ("안녕하세요 여러분!")
- 지역/맛집 키워드를 넣은 질문 2개 (체류시간용)
  예: "혹시 {region_text}에 있는 맛집을 찾고 계신가요?"
  예: "아니면 {region_text} 근처에서 혼밥할 만한 곳을 알아보시나요?"
- 누구와 + 가게명 + 방문 계기 + 첫인상 (3~4줄)
- 서론 마지막에 해시태그 삽입: #지역맛집 #지역맛집2 #가게명 (3~5개)

[2. 서브타이틀 - 호기심/가격 어필]
- 1~2줄로 핵심 어필 (가격, 메뉴 구성, 가성비 등)
  예: "모란역에서 2만원에 훠궈·마라샹궈·꿔바로우까지?… 전성루 훠궈, 이 가격 실화예요?"
- 출처 입력
  (가게 외관 사진)
- 간단 소개글 1~2줄
...
- 출처 입력
  (매장 정보 사진)
- [운영정보] 블록 (아래 형식 그대로, 정보가 없으면 "확인 필요"로 표기):
  위치 : (주소)
  운영시간 : (영업시간)
  라스트오더 : (있으면)
  전화번호 : (번호)
  예약 : (네이버예약/캐치테이블/전화예약 등)
  포장, 배달 : (가능 여부)
  화장실 : (매장 내부/외부/건물 공용)
...
- 출처 입력
  (테이블 사진)
- 좌석 규모/테이블 수/분위기 설명 (2~3줄)
- 추천 용도 (모임/가족/데이트 등) 1줄

[7. 셀프바 & 기본 반찬]
- "셀프바 & 기본 반찬 리뷰" 소제목 (셀프바 없으면 "기본 반찬 리뷰")
- 출처 입력
  (셀프바 또는 반찬 사진)
- 반찬 종류 나열 + 맛 평가 (2~3줄)
...
- 메뉴별로 반복:
  ...
  (음식 사진2)
  추가 감상 (1~2줄)
- 독자 질문 삽입 ("혹시 이 메뉴 드셔본 분?", "어떤 게 더 끌리세요?")
...
- 사진 자리는 반드시 이렇게 표시:
  출처 입력
  (사진 설명 - 괄호 안에 어떤 사진인지 적을 것)
- [사진: ] 형식 사용 금지
- 메인 메뉴 섹션: 메뉴당 "출처 입력" 2~3개
- 전체 "출처 입력" 10~15개

=== 체류시간 극대화 ===
- 서론에 키워드 포함 질문 2개 필수
- 서론 끝에 궁금증 유발 ("근데 이 집 진짜 대박인 게 뭐냐면요...")
- 서브타이틀로 가격/가성비 어필 → 스크롤 유도
- 본론 중간에 독자 질문 2개 이상
- 꿀팁/추천 조합으로 끝까지 읽게 유도
- 마무리에 다음 글 예고
- 3문장 이상 이어지는 문단 금지 (1~2문장마다 줄바꿈)
- 사진 자리 사이에 감성 문장 삽입
...
=== 어미 다양성 ===
- ~었어요, ~더라고요만 반복하지 말 것
- 보이스 가이드 어미를 최대한 돌려쓸 것
- 감탄형/여운형/솔직형/질문형 섞을 것
- 메뉴 리뷰에서 식감/향/비주얼/간 등 감각 교차
...
### 제목 후보
(핵심 키워드 맨 앞 배치, 20~30자 이내, {TITLE_COUNT}개)

### 본문
(위 템플릿 1~10번 구조를 정확히 따를 것)
(서론 끝에 해시태그 포함, 본문 끝에는 해시태그 넣지 말 것)
"""
    return prompt
