...
ENDINGS_POOL = [
    # 세트 A: 밝고 활발한 톤
    {
        ...
        "endings": [
            ...
            "~인 거 있죠!", "~거든요!", "~잖아요!", "~라니까요!",
            "~었다는 거~", "~그 자체였어요!", "~말 다 했죠 ㅎㅎ",
        ],
        "connectors": ["근데", "아 그리고", "여튼", "참고로", "ㅎㅎ 그리고",
                        "그래서요", "아 근데 또", "여기서 포인트는"],
    },
    # 세트 B: 차분하고 담백한 톤
    {
        "name": "담백톤",
    ...
    # 세트 C: 감성적이고 수다스러운 톤
    {
        ...
        "endings": [
            ...
            "~눈물날뻔 ㅠㅠ", "~행복했답니다~", "~었거든요!",
            "~아닌가요?!", "~이게 되나 싶었어요 ㅋㅋ", "~실화에요,,",
            "~난리 났어요 ㅎㅎ", "~감동이었어요 진짜", "~못 참겠더라고요 ㅋㅋ",
            "~인생이었어요,,",
        ],
        "connectors": ["아니 근데", "진짜", "헐 그리고", "ㅋㅋㅋ 그리고", "아 맞다",
                        "그래갖고", "아무튼 결론은", "솔직히 말하면"],
    },
    # 세트 D: 정보형 톤 (리뷰어 느낌)
    {
        "name": "리뷰톤",
        "endings": [
            "~이랍니다.", "~확인했어요.", "~참고하세요~", "~알아두면 좋아요.",
            "~체크해보세요!", "~포인트예요.", "~눈여겨보세요~",
            "~비교해봤어요.", "~차이가 있더라고요.", "~꿀팁이에요!",
            "~알아두면 득템이에요.", "~놓치면 아까워요~",
        ],
        "connectors": ["참고로", "덧붙이면", "한 가지 더", "팁을 드리자면",
                        "비교하자면", "정리하면", "요약하면"],
    },
]

# === 어미 다양성 강화 풀 (톤과 무관하게 랜덤 섞어 사용) ===
DIVERSE_ENDINGS = [
    # 감탄/놀람
    "~깜짝 놀랐어요", "~소름이었어요", "~반전이었어요", "~예상 밖이었어요",
    # 만족/추천
    "~강추드려요", "~후회 없을 거예요", "~꼭 가보세요", "~안 가면 손해에요",
    "~이 가격이면 혜자예요", "~가성비 최고였어요",
    # 감성/여운
    "~생각나는 맛이에요", "~자꾸 떠올라요", "~또 먹고 싶더라고요",
    "~벌써 그리워요,,", "~잊을 수가 없어요",
    # 솔직/담백
    "~나쁘지 않았어요", "~무난했어요", "~호불호 갈릴 수 있어요",
    "~취향 차이인 것 같아요", "~딱 기대한 만큼이었어요",
    # 디테일/묘사
    "~눈으로 먼저 먹었어요", "~향부터 달랐어요", "~식감이 살아있었어요",
    "~한입에 반했어요", "~씹을수록 깊어져요",
    # 에피소드
    "~먹다가 멈출 수가 없었어요", "~사진 찍는 거 까먹었어요 ㅋㅋ",
    "~다 먹고 나서 한참 여운이,,", "~배 터지게 먹었어요",
]
...
def pick_voice_set() -> dict:
    ...
    # 톤 기본 어미 + DIVERSE_ENDINGS에서 랜덤 5개 추가 → 다양성 강화
    bonus_endings = random.sample(DIVERSE_ENDINGS, k=min(5, len(DIVERSE_ENDINGS)))
    mixed_endings = tone["endings"] + bonus_endings
    random.shuffle(mixed_endings)
...
def build_voice_guide(voice: dict) -> str:
    ...
    endings_text = ", ".join(voice["endings"][:10])
